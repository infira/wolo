...
class Http
{
	...
	public static function __callStatic(string $name, array $arguments)
	{
		...
		elseif (in_array($name, ['getPOST', 'setPOST', 'deletePOST', 'unsetPOST', 'existsPOST', 'hasPOST', 'flushPOST'])) {
			$name = substr($name, -4);
			
			return self::post()->$name(...$arguments);
		}
		elseif (in_array($name, ['getGET', 'setGET', 'deleteGET', 'unsetGET', 'existsGET', 'hasGET', 'flushGET'])) {
			$name = substr($name, 0, -3);
			
			return self::url()->$name(...$arguments);
		}
		elseif (in_array($name, ['getFILE', 'setFILE', 'deleteFILE', 'unsetFILE', 'existsFILE', 'hasFILE', 'flushFILE'])) {
			$name = substr($name, -4);
			
			return self::file()->$name(...$arguments);
		}
		elseif (in_array($name, ['getSERVER', 'existsSERVER', 'hasSERVER'])) {
			$name = substr($name, -4);
	 ...
	 */
	#[NoReturn] public static function go(string $link)
	{
	 ...
	 * @param string $link - where to go
	 */
	#[NoReturn] public static function go301(string $link)
	{
	 ...
	 */
	#[NoReturn] public static function goToReferer(string $addExtraToRefLink = '')
	{
